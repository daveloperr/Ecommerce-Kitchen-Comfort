...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['productName']) && isset($_POST['user_id']) && isset($_POST['isActive'])) {
        $productName = $_POST['productName'];
        $user_id = $_POST['user_id'];
        $isActive = $_POST['isActive'];

        // Update isActive in the cart table
        $sql = "UPDATE cart SET isActive =? WHERE product_name =? AND user_id =?";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            $stmt->bind_param("isi", $isActive, $productName, $user_id);
            if ($stmt->execute()) {
                // Return a success response
                http_response_code(200);
                echo json_encode(array("message" => "isActive updated successfully"));
            } else {
